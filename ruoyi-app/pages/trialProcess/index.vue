      ...
      <view class="modal-box">
        ...
        <view class="modal-form">
          ...
          <view class="form-item">
            ...
            <view v-if="userList.length > 0" class="picker-wrap" @tap="showUserPicker = true">
    ...
    <view class="fullpage-modal" v-if="uploadVisible">
      ...
      <scroll-view scroll-y class="fullpage-body">
        ...
        <view class="upload-section">
          ...
          <text class="upload-tip-text">支持图片（JPG、PNG）格式</text>
          ...
          <view class="file-grid" v-if="historyFiles.length > 0">
            ...
            >
              <view class="file-thumb" @tap="previewFile(file)">
                <image
      ...
      uni.chooseImage({
        ...
        success: (res) => {
          ...
          const uploadNext = (i) => {
            ...
            const fileName = 'file_' + Date.now() + '_' + i + (filePath.endsWith('.png') ? '.png' : '.jpg')
            uploadFile(filePath).then(data => {
              const newFile = {
                name: fileName,
                url: data.url || data.fileName || filePath,
    ...
    previewFile(file) {
      if (this.isImageFile(file.name)) {
        const imageUrls = this.historyFiles
          ...
          .map(f => this.baseUrl + f.url)
        uni.previewImage({
          urls: imageUrls,
          current: this.baseUrl + file.url
        })
      }
    },
    shortenName(name) {
...
.modal-form { margin-bottom: 20rpx; }
